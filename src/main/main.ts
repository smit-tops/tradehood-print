...
store.set('dns', process.env.REACT_APP_SENTRY_DSN);
store.set('appVersion', process.env.REACT_APP_VERSION);
...
ipcMain.handle('getSentry', (event, arg) => {
  return {
...
ipcMain.handle('getSelectedPrinters', async (event, arg) => {
  let credentials = store.get('selectedPrinters');
  return credentials;
});
...
const createWindow = async () => {
  ...
  mainWindow = new BrowserWindow({
    show: false,
    icon: getAssetPath('tradehood.png'),
    webPreferences: {
